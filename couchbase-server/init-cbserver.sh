...
if ! [ -f "$FILE" ]; then
  ...
  /opt/couchbase/bin/couchbase-cli collection-manage -c localhost:8091 \
    ...
    --password $COUCHBASE_ADMINISTRATOR_PASSWORD \
    --bucket $COUCHBASE_BUCKET \
    --create-collection audit.userProfiles
  ...
  -d "file:///opt/couchbase/init/sample-customers.json" -g %customerId% -t 4
  ...
  -d "file:///opt/couchbase/init/sample-districts.json" -g %districtId% -t 4
  ...
  -d "file:///opt/couchbase/init/sample-items.json" -g %itemId% -t 4
  ...
  -p $COUCHBASE_ADMINISTRATOR_PASSWORD \
  -b $COUCHBASE_BUCKET \
  --scope-collection-exp sales.stock \
  -d "file:///opt/couchbase/init/sample-stock.json" -g '#UUID#' -t 4
  ...
  /opt/couchbase/bin/cbimport json --format list \
    ...
    -d "file:///opt/couchbase/init/sample-warehouses.json" -g '%warehouseId%' -t 4
  ...
  -d "file:///opt/couchbase/init/sample-projects.json" -g %projectId% -t 4
  ...
  /opt/couchbase/bin/cbimport json --format list \
    ...
    -p $COUCHBASE_ADMINISTRATOR_PASSWORD \
    -b $COUCHBASE_BUCKET \
    --scope-collection-exp audit.inventory \
    -d "file:///opt/couchbase/init/sample-audit-inventory.json" -g %auditId% -t 4
  ...
  /opt/couchbase/bin/cbimport json --format list \
    ...
    -p $COUCHBASE_ADMINISTRATOR_PASSWORD \
    -b $COUCHBASE_BUCKET \
    --scope-collection-exp audit.userProfiles \
    -d "file:///opt/couchbase/init/sample-userProfiles.json" -g %docId% -t 4
